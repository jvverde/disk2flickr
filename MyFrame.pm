# generated by wxGlade 0.6.8 (standalone edition) on Sun Nov 03 21:52:41 2013
#
...
# Menu Bar end
    ...
    $self->{progressBar} = Wx::Gauge->new($self->{backupPanel}, wxID_ANY, 10, wxDefaultPosition,     wxDefaultSize, wxGA_HORIZONTAL);
...
sub __set_properties {
    ...
    $self->SetSize(Wx::Size->new(711, 468));
...
sub __do_layout {
    ...
    $self->{backupSizer}->Add($self->{progressBar}, 0, wxEXPAND, 0);
...
sub do_remove_selected {
    my ($self, $event) = @_;
	return $self->go_remove_selected($event);
...
sub do_remove_all {
    my ($self, $event) = @_;
	return $self->go_remove_all($event);
